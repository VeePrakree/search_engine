...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  if (realpath(test_file.c_str(), resolvedPath) == nullptr) return false;

  char resolvedDir[PATH_MAX];
  if (realpath(root_dir.c_str(), resolvedDir) == nullptr) return false;

  std::vector<std::string> split_file_path;
  boost::split(split_file_path, resolvedPath, boost::is_any_of("/"));

  std::vector<std::string> split_directory_path;
  boost::split(split_directory_path, resolvedDir, boost::is_any_of("/"));

  for (size_t i = 0; i < split_directory_path.size(); i++) {
    if (split_file_path[i] != split_directory_path[i]) {
      return false;
    }
  }

  std::ifstream fileOpener;
  fileOpener.open(resolvedPath);
  return fileOpener.is_open();
}
