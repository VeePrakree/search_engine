...
bool FileReader::ReadFile(string* const contents) {
  ...
  // STEP 1:
  if (!IsPathSafe(basedir_, fname_)) return false;

  int size = 0;
  char* file_str = ReadFileToString(full_file.c_str(), &size);
  if (!file_str) return false;

  *contents = string(file_str, static_cast<size_t>(size));
